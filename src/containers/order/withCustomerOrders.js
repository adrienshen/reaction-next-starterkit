...
import { orderByReferenceId, customersOrders } from "./queries.gql";
  ...
  class WithOrder extends React.Component {
    ...
    render() {
      ...
      const variables = {
        ...
        userId: "asdfddd", // from auth
        shopId: primaryShopId,
        token: routingStore.query.token || null
      };

      return (
        <Query errorPolicy="all" query={customersOrders} variables={variables}>
          {({ loading: isLoading, data: orderData }) => {
            const { order } = orderData || {};

            return <Component {...this.props} isLoadingOrder={isLoading} order={order} />;
          }}
        </Query>
      );
    }
